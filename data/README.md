...
## `/quran/ihtiyaclar.json` (2026-09-10 eklendi)

**`ihtiyac.html` ("Bugün ne okumak istersin?") sayfasının veri kaynağı.** Kullanıcının önerisi: "rastgele ayet" yerine, ihtiyaç/duygu temelli 10 kategoriden (Huzur, Bilgi, Dua, Zor Zaman, Aile, Rızık, Adalet, Sabır, Tevbe, Şükür) birini seçip o temayla ilgili ayetleri, farklı mealleri ve tefsirleri bir arada görmek — SEO değeri de olan ("Kur'an'da sabır ile ilgili ayetler" gibi aramalardan gelen ziyaretçiyi günlük kullanıcıya çevirme) bir tasarım.

**Kritik ilke — bu bir "AI dini hüküm veriyor" aracı DEĞİLDİR:** Sayfa hiçbir yorum/hüküm üretmez; sadece kaynakları (ayet, meal, tefsir) bir araya getirir, sayfanın en üstünde bunu açıkça belirten bir not vardır.

**İçerik ve doğrulama:** 10 kategori × 5 ayet = 50 giriş. Şema sadece `{kategori, sure_id, ayet_bas, ayet_son}` — ayetin Arapça/meal metni JSON'a GÖMÜLMEDİ, sayfa çalışma anında `quran-arabic.json`/`quran-turkish.json`'dan (tıpkı `peygamber.html` gibi) canlı okuyor; bu, gömülü bir kopyanın kaynak düzeltildiğinde bayatlaması riskini (bkz. yukarıdaki meal düzeltmesi krizi) baştan ortadan kaldırıyor. Her ayet önce tema uygunluğu açısından seçildi, sonra sitenin kendi verisinden gerçek metniyle çekilip elle okunarak doğrulandı — bu süreçte **2 hatalı seçim bulunup düzeltildi**: (1) Bakara 2:222 başta "tevbe" kategorisi için düşünülmüştü ama ayetin asıl konusu kadınların ay hali/fıkhi bir hüküm, tevbe sadece kapanış cümlesinde geçiyordu — bağlamından koparılmış olurdu, yerine doğrudan tevbeyle ilgili Tevbe 9:104 kondu. (2) Birkaç ayette (Tâhâ 20:114, Zümer 39:9, Mücadele 58:11, Fâtır 35:28) aranan anahtar ifade ayetin BAŞINDA değil sonunda çıktı — tam ayet metni okunarak doğrulandı, kısmi/yanıltıcı alıntı riski önlendi.

**Her kategorinin bir "anahtar kök"ü var** (`kok` alanı) — sayı iddiaları hiçbiri ezbere yazılmadı, `data/quran/roots-index.json`'dan (bu oturumda daha önce çapraz doğrulanmış) çekildi: صبر(93), شكر(69), دعو(182), عسر(12), ولد(80), رزق(109), عدل(24), توب(69), علم(728), سكن(66). Bu kök, `ara.html?kok=<kök>` deep-link'iyle (bu turda `ara.html`'e eklendi — `oku.html?sure=&ayet=` ile aynı derin bağlantı deseni) kök arama aracına bağlanıyor.

**Farklı mealler ve tefsir:** Sayfa her ayet için isteğe bağlı olarak Elmalılı Hamdi Yazır + Yaşar Nuri Öztürk meallerini (`data/quran/meals/*.json`'dan, `oku.html`'in kullandığı AYNI lazy-fetch deseni) ve İbn Kesîr tefsirini (`raw.githubusercontent.com/spa5k/tafsir_api`'den, yine `oku.html`'in "Tefsirle Oku" modundaki AYNI canlı-çekme mekanizması) gösteriyor — hiçbir yeni veri kaynağı icat edilmedi, sitenin zaten doğrulanmış altyapısı tekrar kullanıldı.

**SEO notu (dürüstçe belirtilmeli):** Sayfa `ihtiyac.html?tur=<kategori>` sorgu parametresi + `history.pushState` ile çalışıyor (peygamber.html'in `?id=` deseniyle tutarlı) — başlık/açıklama JS ile değişiyor. Bu, 10 ayrı statik dosya oluşturmaya göre bakımı çok daha kolay ama arama motorlarının JS-render edilen içeriği ne kadar güvenilir indeksleyeceği bilinmiyor (Google'ın JS çalıştırdığı biliniyor ama garantisi yok). İleride SEO performansı yetersiz kalırsa, 10 kategorinin her biri ayrı, önceden oluşturulmuş statik bir HTML dosyasına dönüştürülebilir.
